...
class Vec2
	...
	public:
		...
		Vec2<T>& operator*= (const T &R);
#ifdef VEC_USE_MAT
		Vec2<T>& operator*= (const Mat2<T> &R);
#endif
		Vec2<T>& operator/= (const Vec2<T> &R);
		Vec2<T>& operator/= (const T &R);
		Vec2<T>& operator++ ();
...
template <class T>
Vec2<T> Vec2<T>::operator+ (const Vec2<T> &R) const
	{
	Vec2<T> c; c.x=x+R.x; c.y=y+R.y; return(c);
	}

template <class T>
Vec2<T> Vec2<T>::operator+ (const T &R) const
	{
...
template <class T>
Vec2<T>& Vec2<T>::operator+= (const Vec2<T> &R)
	{
	x+=R.x; y+=R.y;
	return(*this);
	}
...
template <class T>
Vec2<T>& Vec2<T>::operator+= (const T &R)
	{
	x+=R; y+=R;
	return(*this);
	}
...
#ifdef VEC_USE_MAT
template <class T>
Vec2<T>& Vec2<T>::operator*= (const Mat2<T> &R)
	{
	*this=*this*R;
	return(*this);
	}
...
template <class T>
bool Vec2<T>::operator== (const Vec2<T> &R) const
	{
