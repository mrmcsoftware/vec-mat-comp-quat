...
*/
		...
		inline void setFormat(int f) // 0: 1+j2  1: 1,2i  2: 1+2i  3: 1,2
			{
			Comp<T>::c_format=f;
			}
...
template <class T>
int Comp<T>::c_format = 0;

template <class T>
std::ostream& operator<< (std::ostream &os, const Comp<T> &R)
	{
	switch (Comp<T>::c_format)
		{
	 	case 0: os << "(" << R.r << "+j" << R.i << ")"; break;
		case 1: os << "(" << R.r << "," << R.i << "i)"; break;
		case 2: os << "(" << R.r << "+" << R.i << "i)"; break;
		case 3: os << "(" << R.r << "," << R.i << ")"; break;
		}
	return(os);
	}
