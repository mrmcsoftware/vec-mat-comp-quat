...
class Vec2
	...
	friend Vec2 operator+ (const Vec2 &L, const T &R)
		{
		Vec2 c; c.x=L.x+R; c.y=L.y+R; return(c);
		}

	friend Vec2 operator+ (const T &L, const Vec2 &R)
		{
	...
	friend Vec2& operator+= (Vec2 &L, const T &R)
		{
		L.x+=R; L.y+=R;
		return(L);
		}
...
#ifdef VEC_USE_MAT
	friend Vec2& operator*= (Vec2 &L, const Mat2<T> &R)
		{
		L=L*R;
		return(L);
		}
	...
	friend Vec2 operator++ (Vec2 &L, int)
		{
		Vec2 c=L;
		L.x++; L.y++;
		return(c);
